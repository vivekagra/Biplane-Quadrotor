...
class PhysicsSim():
    def __init__(self, init_pose=None, init_velocities=None, init_angle_velocities=None, runtime=5.):
        ...
        bounds = 50
        self.lower_bounds = np.array([-bounds, -bounds, -bounds, -np.pi, -np.pi, -np.pi, -bounds, -bounds, -bounds, -bounds, -bounds, -bounds])
        self.upper_bounds = np.array([bounds,  bounds,  bounds,  np.pi,  np.pi,  np.pi, bounds,  bounds,  bounds,  bounds,  bounds,  bounds])
    ...
    def get_state(self):
        state=np.concatenate([self.pose, self.v, self.angular_v])
        return state

    def set_state(self, set_state):
        self.pose = set_state[:6]
        self.v = set_state[6:9]
        self.angular_v = set_state[9:12] 
